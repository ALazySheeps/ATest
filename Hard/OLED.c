...
void OLED_ShowCN_plus(uint8_t Line, uint8_t Column, uint8_t Num)  //32x32
{      	
	uint8_t i;
	uint8_t wide = 32; // 字宽
	
	for (uint8_t page = 0; page < 4; page++) {
		OLED_SetCursor((Line-1)*4 + page, (Column-1)*wide); // 设置光标位置
		for (i = 0; i < wide; i++) {
			OLED_WriteData(OLED_F32x32[Num][i + page*wide]); // 显示每一部分内容
		}
	}
}

void OLED_ShowCN_plus_pro(uint8_t Line, uint8_t Column, uint8_t Num) //64x64
{
    uint8_t i;
    uint8_t wide = 64;
    
    // OLED 每页是8行，这里有64行，所以需要8页来显示
    for (uint8_t page = 0; page < 8; page++) {
        OLED_SetCursor((Line-1)*8 + page, (Column-1)*wide); // 设置光标位置
        for (i = 0; i < wide; i++) {
            OLED_WriteData(OLED_F64x64[Num][i + page*wide]); // 显示每一部分内容
        }
    }
}
